...
class DuckDuckGo(ConroyPlugin):
    ...
    def search(self, query):
        try:
            ...
            title = truncate_newline(result.find_element_by_css_selector('div > h2 > a.result__a').text)
            url = result.find_element_by_css_selector('div > h2 > a.result__a').get_attribute('href')
            description = truncate_ellipses(truncate_newline(result.find_element_by_css_selector('div > div.result__snippet.js-result-snippet').text), 384)
